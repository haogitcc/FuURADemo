...
int selectedIndex_s = 0;

@interface SerialDeviceViewController ()<RDRscMgrInterfaceDelegate>{
...
@implementation SerialDeviceViewController
...
//    self.fastSearchBtn.frame = CGRectMake(self.fastSearchBtn.frame.origin.x, self.fastSearchBtn.frame.origin.y-50, self.view.frame.size.width, self.view.frame.size.height);
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cableDisconneted) name:@"CableDisconneted" object:nil];
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(timeoutReadAgain) name:@"TimeoutReadAgain" object:nil];
...
- (IBAction)baudratelistAction:(id)sender {
    
    self.baudratelistBtn.tag = 1;
    self.regionlistBtn.tag = 0;
    
    ListPickerViewController *testViewController = [[ListPickerViewController alloc] init];
    testViewController.delegate = self;
    testViewController.listType = @"BaudRate";
    self.listPickerPopover = [[UIPopoverController alloc] initWithContentViewController:testViewController];
    
    self.listPickerPopover.popoverContentSize = CGSizeMake(300.0, 200.0);
    [self.listPickerPopover presentPopoverFromRect:[(UIButton *)sender frame]
                                            inView:self.view
...
- (IBAction)regionlistAction:(id)sender {
    
    self.baudratelistBtn.tag = 0;
    self.regionlistBtn.tag = 1;
    
    ListPickerViewController *testViewController = [[ListPickerViewController alloc] init];
    testViewController.delegate = self;
    testViewController.listType = @"Region";
    self.listPickerPopover = [[UIPopoverController alloc] initWithContentViewController:testViewController];
...
- (IBAction)connectAction:(id)sender {
    ...
    NSLog(@"*******Connect to Redpart Cable******");
...
//    [HUD show:YES];
    ...
    uint32_t baudrate = [self.baudrateTxtfield.text intValue];;
    ...
    if ([self.tranceportlogBtn isOn]) {
        ...
        tb.listener = serialDeviceSerialPrinter;
...
//        bool enablePreamble = TRUE;
//        ret = TMR_paramSet(rp, TMR_PARAM_ENABLE_PREAMBLE, &enablePreamble);
        //rp->u.serialReader.supportsPreamble = false;
        
        ret = TMR_connect(rp);
        if (TMR_SUCCESS != ret)
        {
            NSLog(@"COnnect Status:%s", TMR_strerr(rp, ret));
            [writetplog writeData:[[NSString stringWithFormat:@"%s\n", TMR_strerr(rp, ret)] dataUsingEncoding:NSUTF8StringEncoding]];
        }
       ...
       // //[writelog writeData:[[NSString stringWithFormat:@"region ----- %d\n",region] dataUsingEncoding:NSUTF8StringEncoding]];
        ...
        if (TMR_REGION_NONE == region)
        {
            ...
            if (TMR_SUCCESS != retStatus)
                NSLog(@"Supported Region Status :%s", TMR_strerr(rp, ret));
            
            //NSLog(@"***** Region len -- %d", regions.len);
            ...
            if (TMR_SUCCESS != retStatus)
                NSLog(@"Region Param Set Status :%s", TMR_strerr(rp, ret));
...
void serialDeviceSerialPrinter(bool tx, uint32_t dataLen, const uint8_t data[],
...
-(void) discontBtnAction{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"********* STOP READING IN MAIN Queue***********");
                ...
                else
                {
                    while (TMR_SUCCESS == TMR_hasMoreTags(rp))
                    {
                        ...
                        if (![allepclblsArray containsObject:epcString]){
                            ...
                            [rrso setepcServiceName:[[services objectAtIndex:selectedIndex_s] objectAtIndex:1]];
...
-(void)asyncRead
{
    ...
    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (TMR_SUCCESS != ret)
        {
            NSLog(@"addReadListener Status:%s", TMR_strerr(rp, ret));
        }
        
        ret = TMR_addReadExceptionListener(rp, &reb);
        if (TMR_SUCCESS != ret)
        {
            NSLog(@"addReadExceptionListener Status:%s", TMR_strerr(rp, ret));
        }
...
void callback(TMR_Reader *reader, const TMR_TagReadData *t, void *cookie){
    ...
    @autoreleasepool
    {
        ...
        alltotalTags = alltotalTags +[epcCount intValue];
        //NSLog(@"read ---- EPC:%@  ReadCount:%d",[rrso getepclblTxt],[rrso getepcTagCount]);
    }
    
   // NSLog(@"Total Tags read --- %d  ", alltotalTags);
...
void exceptionCallback(TMR_Reader *reader, TMR_Status error, void *cookie)
{
    ...
    NSLog(@"**** exceptionCallback -- %@",exceptionlbl);
    
    /*
    
    if (error == TMR_ERROR_TIMEOUT ||
        error == TMR_ERROR_CRC_ERROR ||
        error == TMR_ERROR_TAG_ID_BUFFER_NOT_ENOUGH_TAGS_AVAILABLE ||
        error == TMR_ERROR_DEVICE_RESET)
    {
        
          [[NSNotificationCenter defaultCenter] postNotificationName:@"STOPREADACTION"  object:nil];
...
-(void)timeoutReadAgain
{
    NSLog(@"**** TIME OUT READ AGAIN ***********");
    ...
    UIApplication  *app = [UIApplication sharedApplication];
    bgTask = [app beginBackgroundTaskWithExpirationHandler:^{
        NSLog(@"***** APP BACK GROUND TAS END");
        [app endBackgroundTask:bgTask];
    }];
     */
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"TimeoutReadAgain" object:nil];
...
-(void) stopReadResults{
    ...
    //dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSLog(@"**** Stop read Action 4 start*****");
        [self.silenceTimer invalidate];
        
        
        if (rp != NULL) {
            //NSLog(@"**** Stop read Action 4 1*****");
            ret = TMR_stopReading(rp);
            //NSLog(@"**** Stop read Action 4 2*****");
            ret = TMR_removeReadListener(rp, &rlb);
            //NSLog(@"**** Stop read Action 4 3*****");
            ret = TMR_removeReadExceptionListener(rp, &reb);
            //NSLog(@"**** Stop read Action 4 4*****");
        }
        NSLog(@"**** Stop read Action 4 stop*****");
