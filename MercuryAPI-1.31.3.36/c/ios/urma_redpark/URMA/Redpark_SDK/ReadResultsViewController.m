...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(displayException) name:@"DisplayException" object:nil];
...
-(void) startTimer{
    self.silenceTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self
...
-(void) RefreshReadResults{
    ...
    timeInSec = [NSString stringWithFormat:@"%d",[timeInSec intValue] + 1];
    [self callFootorSummary];
...
- (UITableViewCell *)tableView:(UITableView *)table_View cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell loadreadResultTableViewCell:[rrvo getepclblTxt] date:[rrvo getTimestampHigh] rssi:[rrvo getRssi] readcout:[rrvo getepcTagCount] antenaa:[rrvo getAntenna] phase:[rrvo getPhase] frequency:[rrvo getFrequency] protocal:[rrvo getProtocol] serviceType:startIndex+1];
    return cell;
}
